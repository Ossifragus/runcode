...
- =\runPythonBatch[Arg1][Arg2]= runs an external [[https://www.python.org/][Python]] code file in batch mode
  ...
  means that once a Python session ends, the working environment (variable,
...
** Revisions
- v2.5, June 2026:
  1. Checksum-based cache invalidation: source files are now fingerprinted
     with MD5; a script is re-executed only when its source has changed (or
     when the output file is missing), avoiding unnecessary reruns on
     unchanged code.
  2. New ~wait_for_rserver.py~: polls a talk2stat server until it accepts
     connections, eliminating the server-startup race condition in ~nohup~
     mode. Supports R, Python, Julia, and Matlab via a command-line argument.
  3. New ~runcode-Makefile.sample~: a sample Makefile encoding three best
     practices for ~nohup~ builds — quit any old server before starting
     fresh, wait for the server to be ready (via ~wait_for_rserver.py~), and
     send a sync barrier after the first draft pass so all queued scripts
     finish before results are cached.
  4. New ~consolidate.py~: produces a self-contained copy of a runcode
     project with all cached outputs inlined into the =.tex= source, so the
     result compiles without talk2stat or any language runtime. Handles all
     language variants (~\runRChunk~, ~\runRIncOut~, ~\includeOutput~,
     ~\inlnR~, etc.) and replaces ~\usepackage{runcode}~ with a minimal
     tcolorbox + listings shim.  Motivated by the Overleaf use case but
     general-purpose.
- v2.4.1, June 10, 2026: bug fixes in ~\writeChunk~: (1) replaced
  =mkdir -p= with Python =os.makedirs= for cross-platform subdirectory
  creation (=mkdir -p= is unavailable on Windows and sensitive to
  =\filename@parse= expansion differences across TeX distributions);
  (2) fixed stream exhaustion when more than 16 ~\showChunk~ calls are
  made in one document (streams are now pre-allocated at package load
  time); (3) removed erroneous =\write18{ad ...}= call in ~\showChunk~.
  Added AUCTeX magic comment to example files so Emacs users get
  =--shell-escape= automatically.
...
- v2.0, June 23, 2023: add =\runCodeIncOut= command that runs an external code and
  embeds the output and add some language specific shortcuts; update
